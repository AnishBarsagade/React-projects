...
const App = () => {
  ...
  return (
    <div className="bg-black overflow-auto p-4 h-screen text-white">
      <div className="flex grow flex-wrap gap-4 p-2">{printUserData}</div>
      <div className="flex justify-center gap-6 items-center p-4">
        <button
          style={{ opacity: index == 1 ? 0.5 : 1 }}
          className="bg-amber-400 text-sm cursor-pointer active:scale-95 text-black px-4 py-2 font-semibold rounded"
          onClick={() => {
            if (index > 1) {
              setIndex(index - 1);
              setUserData([]);
